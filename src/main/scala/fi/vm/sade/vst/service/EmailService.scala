...
class EmailService(casUtils: CasUtils,
  ...
  def sendEmails(releases: Seq[Release], eventType: EmailEventType)(implicit au: AuditUser): Seq[LuoViestiSuccessResponse] = {
    ...
    try {
      val response = viestinvalitysClient.luoViesti(Viesti.builder()
      ...
      System.out.println("Lähetystunniste: " + response.getLahetysTunniste)
    } catch {
      case e: BuilderException => {
        val virheet = e.getVirheet
        System.out.println(virheet.iterator().next())
      ...
      if (releaseSetsForUsers.isEmpty) {
        ...
        try {
          val luoLahetysResponse = viestinvalitysClient.luoLahetys(Lahetys.builder()
            ...
            .withLahettavaPalvelu("virkailijan-tyopoyta")
    ...
    val viestit = recipientsByLocalizedReleaseSet.map{ case (releaseSet, recipients) => {
      ...
      recipients
        ...
        .map(recipients => Viesti.builder()
          ...
          .withVastaanottajat(recipients.map(email => Vastaanottajat.builder.withVastaanottaja(Optional.empty.asInstanceOf[Optional[String]], email).build().get(0)).toList.asJava)
